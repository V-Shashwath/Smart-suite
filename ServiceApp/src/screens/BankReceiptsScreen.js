...
const BankReceiptsScreen = () => {
  ...
  const [headerData, setHeaderData] = useState({
    date: '',
    bankAccount: 'Bank', // Default to Bank account
  });
  ...
  // Instrument type options - uppercase display
  ...
  const handleAddRow = () => {
    ...
    const customerName = customerData.customerName || (bodyItems.length > 0 ? bodyItems[0].account : '');
    const newItem = {
      id: Date.now(),
      sno: String(bodyItems.length + 1),
      account: customerName, // Use existing customer name
      instrumentType: '',
      instrumentNo: '',
      instrumentDate: '',
      instrumentDetails: '',
    ...
    setBodyItems([...bodyItems, newItem]);
  };

  const handleDeleteRow = (index) => {
    // Always maintain at least one row
    if (bodyItems.length <= 1) {
      // If it's the last row, show clear confirmation
      Alert.alert(
        'Clear Customer',
        ...
        [
          { text: 'Cancel', style: 'cancel' },
          {
            ...
            style: 'destructive',
            onPress: () => {
              // Reset to empty row but keep one row
              setBodyItems([{
                id: Date.now(),
                sno: '1',
                account: '',
  ...
  const handleCellChange = (rowIndex, columnKey, value) => {
    ...
    if (rowIndex < 0 || rowIndex >= bodyItems.length) {
      console.error(`Invalid rowIndex: ${rowIndex}, bodyItems length: ${bodyItems.length}`);
      return;
    }
    
    const newItems = [...bodyItems];
    
    // Ensure the row exists before trying to set property
        ...
        if (result.success && result.data) {
          ...
          setCustomerData({
            ...
            whatsappNo: customer.WhatsAppNo || customer.MobileNo,
              ...
              // Update all existing rows with customer name (always maintain at least one row)
              const updatedItems = bodyItems.length > 0 
  ...
  const getInvoiceData = useCallback(() => ({
    ...
    customerData: {
      customerName: headerData.bankAccount,
  ...
  const handleSaveInvoice = async () => {
    // Check if at least one row has a customer name
    const hasCustomer = bodyItems.length > 0 && bodyItems.some(item => item.account && item.account.trim() !== '');
    if (!hasCustomer) {
      Alert.alert(
        'No Customer',
  ...
  const handleExit = () => {
    // Check if there are any rows with customer name
    const hasCustomer = bodyItems.length > 0 && bodyItems.some(item => item.account && item.account.trim() !== '');
    const hasUnsavedChanges = hasCustomer && !isInvoiceSaved;
